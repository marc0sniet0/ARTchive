...
const CONTINUE_URL = "./SalaMinimalismo.html";
...
const MOVEMENT_LABEL = "Arte conceptual";


const CURATED_CONCEPTUAL = [
  { artist: "Barbara Kruger", title: "Have Me Feed Me Hug Me Love Me Need Me" },
  { artist: "Mel Bochner", title: "Untitled (5-Part Progression)" },
  { artist: "Mel Bochner", title: "3-Way Fibonacci Progression" },
  { artist: "Sol LeWitt", title: "Wall Drawing #128 (Ten Thousand Random Not Straight Lines)" },
  { artist: "Jenny Holzer", title: "Untitled (C-4)" },
  { artist: "Lawrence Weiner", title: "Statement" },
  { artist: "Piero Manzoni", title: "Achrome" },
  { artist: "On Kawara", title: "Today Series, \"Tuesday\"" },
  { artist: "Marcel Duchamp", title: "Seated in a Bathtub" },

  { artist: "Piero Manzoni", title: "Artist's Shit" },   
  { artist: "Joseph Beuys", title: "Fat Chair" }                 
];
...
async function aicSearchMany({ artist, title }, limit = 10){
  // Extra: “Conceptual” ayuda a que AIC no te devuelva cosas random de otro movimiento
  const q = `${title} ${artist} conceptual`;
...
function scoreCandidate(candidate, wantArtist, wantTitle){
  ...
  // bonus suave si el estilo sugiere conceptual (cuando exista)
  const st = normalize(candidate?.style_title);
  if (st.includes("concept")) score += 2;

  return score;
}

async function pickBestNonDuplicate(candidates, { artist, title }){
  ...
  for (const c of ranked){
    ...
    const url = aicImageUrl(c.image_id, 1600);
...
async function buildArtwork(item){
  ...
  const candidates = await aicSearchMany(item, 14);
  const chosen = await pickBestNonDuplicate(candidates, item);
  ...
  const unified = {
    ...
    fallbackDesc: `Obra representativa del ${MOVEMENT_LABEL}.`
  };
...
async function loadWorks(){
  ...
  if (loadingTitle) loadingTitle.textContent = `Cargando obras de ${MOVEMENT_LABEL}…`;
  if (loadingSub) loadingSub.textContent = "AIC + Harvard";

  const built = [];
  for (const item of CURATED_CONCEPTUAL){
    try{
