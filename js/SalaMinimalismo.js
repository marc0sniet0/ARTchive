// ====== MINIMALISMO (misma estructura, mismas funciones, mismo flujo) ======
...
// Meto más de 10 para asegurar que, aunque algunas no estén en AIC o no tengan imagen,
// el carrusel se quede con una selección limpia (sin duplicados y con imagen sí o sí).
...
function scoreCandidate(candidate, wantArtist, wantTitle){
  ...
  // títulos minimalistas suelen ser "Untitled", así que esto puede dar falsos positivos;
  // aún así sumo menos peso al título si es "untitled".
  ...
  // bonus suave si el estilo/clasificación suena a minimal
...
function pickBestNonDuplicate(candidates, { artist, title }){
  ...
  const ranked = [...candidates]
    .filter(c => c?.image_id) // <-- adiós obras sin imagen
